 ...
 */
async function getSquadOverview(event, squadId, userId) {
  console.log("[squads-detail] Fetching overview for squad:", squadId);
  ...
  if (memberCheck.rows.length === 0) {
    console.log("[squads-detail] Usuário não é membro do workspace");
...
exports.handler = async (event) => {
  try {
    ...
    // Extract squad ID from query params (Netlify redirect) or path
    let squadId = event.queryStringParameters?.id;
    
    if (!squadId) {
      // Try to extract from path as fallback
      const pathParts = event.path.split("/");
      squadId = pathParts[pathParts.length - 2] || pathParts[pathParts.length - 1];
    }
    ...
    // Route based on method and action/path
    const method = event.httpMethod;
    const action = event.queryStringParameters?.action;
    const isOverview = action === "overview" || event.path.includes("/overview");

    if (method === "GET" && isOverview) {
      return await getSquadOverview(event, squadId, userId);
